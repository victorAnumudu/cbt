...
export default function Calculator() {
  ...
  const handleArithmetic = (value: string): void => {
    console.log(value);
    let notNum = ["CE", "Ans", "Del", "Enter", "Backspace", "+", "/", "*", "-"];
    if (!notNum.includes(value)) {
      setArithmetic((prev) =>
        prev == "0" && value == "0" ? "0" : `${prev != "0" ? prev : ""}${value}`
      );
    } else {
      if (value == "CE") {
        // clears the screen
      ...
      } else if (value == "Ans" || value == "Enter") {
        // Solves and return result
        let valueToSolve = arithmetic.split("");
        let containsOperand = ["+", "/", "*", "-"].includes(
          valueToSolve[arithmetic.length - 1]
        ); // checks if the last entry is an operand
...
//   useEffect(() => {
//     let keyboardEvent = document.addEventListener("keyup", (e) => {
//       let validKeys = calBtn.map((item) => item.value);
//       validKeys.push("Enter", "Backspace");
//     //   console.log(typeof e.key, e.key, e, "event");
//       if (validKeys.includes(e.key)) {
//         handleArithmetic(e.key);
//       }
//     });
//     return () => {
        ...
        <div className="grid grid-cols-4 gap-2">
          {calBtn.map((item, index) => {
            return (
              <CustomButton
                key={index}
                className={`w-full h-10 flex justify-center items-center rounded-lg text-black bg-white ${
                  (item.name == "CE" || item.name == "Ans") && "col-span-2"
                } `}
                value={item.value}
                name={item.name}
                onClick={() => {
                  handleArithmetic(item.value);
                }}
              />
            );
          })}
